...
def get_floots():
    ...
    allFloots = db.get_floots()
    allDictFloots = []
    for floot in allFloots:
        allDictFloots.append(floot.to_dictionary())
    return allDictFloots
...
def create_floot(request_body):
    ...
    if "message" in request_body.keys() and "username" in request_body.keys():
        ...
        return HTTPError(400, "Message or Username fields missing from floot post")
...
def delete_floot(floot_id, request_body):
    ...
    * If everything worked fine and the floot was successfully deleted, this
    ...
    if not db.has_floot(floot_id):
        return HTTPError(404, f"No floot with id {floot_id} in database")
    elif "username" not in request_body.keys():
        return HTTPError(400, "Username field missing from floot post")
    elif request_body["username"] != db.get_floot_by_id(
            floot_id).get_username():
        return HTTPError(401, "You cannot delete a floot you did not create")
    else:
...
def get_comments(floot_id):
    ...
    if not db.has_floot(floot_id):
        ...
        for comment in comments:
            commentsDict.append(comment.to_dictionary())
        return commentsDict
...
def create_comment(floot_id, request_body):
    ...
    if not db.has_floot(floot_id):
        return HTTPError(404, f"No floot with id {floot_id} in database")
    elif "username" not in request_body.keys() or "message" not in \
            request_body.keys():
        return HTTPError(400, "Message or username fields missing from floot "
                              "post")
    else:
        floot = db.get_floot_by_id(floot_id)
        flootComment = FlootComment(request_body["message"], request_body[
            "username"])
        floot.create_comment(flootComment)
        return flootComment.to_dictionary()
    ...
    * Otherwise, if everything works successfully, return "OK".
    ...
    for comment in comments:
        if comment.get_id() == comment_id:
            outComment = comment
    commentIDs = [comment.get_id() for comment in comments]

    if comment_id not in commentIDs:
        return HTTPError(404, f"No comment with id {comment_id} in database")
    elif request_body["username"] != outComment.get_author():
        return HTTPError(401, "You cannot delete comment as a non-author")
    else:
        floot.delete_comment(outComment, request_body["username"])
        return "OK"
